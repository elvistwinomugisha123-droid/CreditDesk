...
export default async function ReceivablesPage() {
  ...
  const entries = await prisma.ledgerEntry.findMany({
    where: { userId: user.id, type: "RECEIVABLE" },
    include: {
  ...
  return (
    <div className="px-5 py-5 space-y-5">
      {/* Summary Banner */}
      <div className="bg-gradient-to-r from-green-500 to-brand-green rounded-2xl p-5 text-white">
        <div className="flex items-center gap-2 mb-2">
          <ArrowDownLeft className="h-5 w-5" />
          <p className="text-sm font-medium text-white/80">Money Owed to You</p>
        </div>
      ...
      <div className="space-y-3">
        {entries.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-400 text-sm">No receivables yet</p>
